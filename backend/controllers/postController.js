    ...
    const getPostById = asyncHandler(async (req, res) => {
      // console.log('getPostById çağrıldı. Gönderi ID:', req.params.id); // KALDIRILDI
      // console.log('req.user:', req.user); // KALDIRILDI
      const post = await Post.findById(req.params.id).populate('author', 'username email');
      // console.log('Post.findById sonucu:', post); // KALDIRILDI
      ...
      if (req.user && req.user.role === 'admin') {
        // console.log('Debug: Kullanıcı admin, erişime izin verildi. Gönderi gönderiliyor.'); // KALDIRILDI
      ...
      if (!post.isPublished) {
        if (req.user && post.author && req.user.id.toString() === post.author._id.toString()) {
          // console.log('Debug: Kullanıcı yazar, yayınlanmamış gönderiye erişime izin verildi.'); // KALDIRILDI
          res.status(200).json(post);
          return; // Yazar ise burada bitir
        } else {
          // console.log('Debug: Yayınlanmamış gönderi ve kullanıcı yazar/admin değil. 403 Forbidden.'); // KALDIRILDI
          res.status(403);
      ...
      // console.log('Debug: Gönderi yayınlanmış veya yetkili kullanıcı. Erişime izin verildi.'); // KALDIRILDI
      res.status(200).json(post);
    });
